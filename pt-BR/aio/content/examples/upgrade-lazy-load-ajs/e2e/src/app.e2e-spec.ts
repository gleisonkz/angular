...
describe('Lazy Loading AngularJS Tests', () => {
  ...
  async function loadAngularJS() {
    ...
    // Might have to re-insert the 'NG_DEFER_BOOTSTRAP!' if the name has been changed since
    // protractor loaded the page
    ...
    await browser.wait(
        ExpectedConditions.presenceOf(element(by.css('app-root app-angular-js'))), 5000,
        'AngularJS app');
    ...
    // Based on https://github.com/angular/protractor/blob/5.3.0/lib/browser.ts#L950-L969
    {
      ...
      await browser.executeScriptWithDescription(
          // TODO: must manually assign __TESTABILITY__NG1_APP_ROOT_INJECTOR__
          // (https://github.com/angular/angular/issues/22723)
          `window.__TESTABILITY__NG1_APP_ROOT_INJECTOR__ = angular.resumeBootstrap(arguments[0]) ` +
              `|| angular.element('app-angular-js').injector();`,
          'resume bootstrap', moduleNames);
    }
