angular.module('phonecatApp').animation('.phone', function phoneAnimationFactory() {
  return {addClass: animateIn, removeClass: animateOut};
  ...
  function animateIn(element: JQuery, className: string, done: () => void) {
    ...
    element.css({display: 'block', position: 'absolute', top: 500, left: 0})
        .animate({top: 0}, done);
  ...
  function animateOut(element: JQuery, className: string, done: () => void) {
    ...
    element.css({position: 'absolute', top: 0, left: 0}).animate({top: -500}, done);
